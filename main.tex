...
\begin{proof}
Since $-1$ is a quadratic residue, we may solve $x^2+y^2=np$ for some $1\le n < p/2$. If $n=1$ we're done. Otherwise, let $X,Y$ be representatives of $x,y$ modulo $n$ with $\abs{X},\abs{Y} \le n/2$. Then we have $X^2+Y^2 = nk$, for some $1\le k\le n/2$. Now
...
\begin{fact*}
    For $\a=(\alpha)$ we have $N(\a) = \abs{N_{K/\Q}(\alpha)}$.
\end{fact*}


\begin{fact*}
$K$ has class number $1$ iff $\OO_K$ is a PID iff $\OO_K$ is a UFD.
\end{fact*}
...
%                   apparently shown in https://wstein.org/129-05/final_papers/Steven_Sivek.pdf
%                   Kummer's lemma is also proven in Swinnerton-Dyer's book using class field theory. He also gives an elementary proof, due to Kummer, which is used in Lean. Asking chatgpt to write a latex file proving this in a human readable style works fine when giving as input the S-D book, the lean formalization, and the paper on this lean formalization :)
\end{proof}
...
\begin{proof}
We have 
\[\tau (p^*:q) \equiv \tau (p^*)^{\frac{q-1}{2}}=\tau (\tau^2)^{\frac{q-1}{2}}=\tau^q \equiv \sum_{a\in\F_p} (a:p)\zeta^{aq}=
\sum_{a\in\F_p} (aq:p)\zeta^{a}=(q:p)\tau \mod q\]
Multiply by $\tau$ to remove this term.
\end{proof}
...
\newpage
\section{The Diophantine equation \texorpdfstring{$y^2=x^3-2$}{}}

Let $x,y\in\Z$ solve $y^2=x^3-2$. We wish to show that $x=3,y=\pm5$. We rewrite 
\[
x^3 = (y+\sqrt{-2})(y-\sqrt{-2})\]
This suggests working in the ring $\Z[\sqrt{-2}]$. This ring is Euclidean, and so a UFD. Let us show that $y+\sqrt{-2},y-\sqrt{-2}$ are coprime. Suppose, for the sake of contradiction, that we have $\pi \mid y+\sqrt{-2}$ and $\pi \mid y-\sqrt{-2}$ for some prime element $\pi$ of $\Z[\sqrt{-2}]$. Then $\pi \mid 2y$, so either $\pi\mid 2$ or $\pi \mid y$. Now $\sqrt{-2}$ is a prime element of $\Z[\sqrt{-2}]$, so in either case $\pi\sim \sqrt{-2} \mid y$. Hence $2\mid y$. Thus $2\mid x$. Modulo four, we get $0\equiv y^2 = x^3 - 2 \equiv 2 \mod 4$, a contradiction. \\
Since $y+\sqrt{-2}, y-\sqrt{-2}$ are coprime, we have
\[y\pm \sqrt{-2} = \eps_{\pm} z_{\pm}^3\]
for units $\eps$ and some $z\in\Z[\sqrt{-2}]$. The only units are $\pm1$, which are cubes, so we may remove the $\eps$ term. So let us consider \[y+\sqrt{-2} = (a+b\sqrt{-2})^3 = (a^3-6ab^2) + \sqrt{-2}(3a^2b-2b^3)\]
We get $1=3a^2b-2b^3$, so $b=\pm 1$. Also, modulo three, we have $b\equiv 1 \mod 3$. So $b=1$, and thus $a=\pm1$. There are two solutions for $y$, namely $\pm 5$, and $x=3$.


\begin{exer*}
    Solve $y^2=x^3-1$ in integers.
\end{exer*}


\newpage
\section{Sums of Four Squares}

\begin{fact*}
Every positif integer is a sum of four integer squares.
\end{fact*}

\begin{proof}
Step 1. Considering that the norm on quaternions is multiplicative, it suffices to show that any odd prime is a sum of four integer squares. \\
Step 2. By the pigeonhole principle, we may solve $1+x^2+y^2\equiv 0\mod p$ with $0\le x,y < p/2$. \\
Step 3. Let $n\ge 1$ be minimal such that $np=x^2+y^2+z^2+w^2$ is the sum of four squares. We have $n<\frac{3}{4}p$. We cannot have $n$ even. Indeed, if $n$ were even, wlog $x,y$ have the same parity and so do $z,w$. Then \[\frac{n}{2}p = \left(\frac{x+y}{2}\right)^2 + 
\left(\frac{x-y}{2}\right)^2 + 
\left(\frac{z+w}{2}\right)^2 +
\left(\frac{z-w}{2}\right)^2\]
Step 4. Suppose $n\ge 3$ is odd. Let $X,Y,Z,W$ be the representatives of $x,y,z,w$ modulo $n$ inside $(-n/2,n/2)$. Then $X^2+Y^2+Z^2+W^2 = kn$ for some $0<k<n$. Now we have
...
$C=xZ-zX+wY-wY$,
$D=yZ-zY+xW-wX$
are all divisible by $n$. Thus \[kp = \left(\frac{A}{n}\right)^2 + \left(\frac{B}{n}\right)^2 + \left(\frac{C}{n}\right)^2 + \left(\frac{D}{n}\right)^2\]
is a smaller solution.
\end{proof}


\newpage
\section{\texorpdfstring{$p$}{}-adics}

\begin{fact*}
Let $f\in\Z[x_1,\dots,x_m]$. Then $f$ is soluble in $\Z_p$ iff $f$ is soluble in $\Z/p^n$ for all $n$.
\end{fact*}


\begin{fact*}
Units in $\Z_p$ are precisely those not divisible by $p$.
\end{fact*}


\begin{fact*}
Rationality $\iff$ eventual periodicity of $p$-adic expansion.
\end{fact*}


\begin{fact*}
The non-zero ideals of $\Z_p$ are precisely $p^n\Z_p$ and $\Z_p/p^n\Z_p\equiv\Z/p^n\Z$.
\end{fact*}


\newpage
\section{The Diophantine equation \texorpdfstring{$x^2+7=2^n$}{}}

Let $x,n$ be integers with $x^2+7=2^n$. We claim that $(x,n) = (\pm1,3),(\pm3,4),(\pm5,5),(\pm11,7),(\pm181,15)$.


First, if $n=2k$ is even, then $7=(2^k+x)(2^k-x)$, so $2^k\pm x=7$ and $2^k\mp x=1$, yielding The solution $(\pm3,4)$. In general, we have $(x+\sqrt{-7})(x-\sqrt{-7})=2^n$. Suggesting to work in the ring of integers of $\Q(\sqrt{-7})$. This is Euclidean, so a UFD. Now $\sqrt{-7}, \dfrac{1\pm\sqrt{-7}}{2}$ are non-associate irreducible elements, $\pm1$ are the only units, and $2=\dfrac{1+\sqrt{-7}}{2}\dfrac{1-\sqrt{-7}}{2}$. We may assume $n\ge 5$ is odd, and of course $x$ is odd. 
We have $\dfrac{x + \sqrt{-7}}{2} \cdot \dfrac{x - \sqrt{-7}}{2} = \left(\dfrac{1+\sqrt{-7}}{2} \right)^m \left(\dfrac{1-\sqrt{-7}}{2} \right)^m$, where $m=n-2$. We see that $\dfrac{x\pm\sqrt{7}}{2}$ must be coprime and conjugate, so either
$\dfrac{x + \sqrt{-7}}{2} = \pm\alpha^m$ and $\dfrac{x - \sqrt{-7}}{2} = \pm\beta^m$
or 
$\dfrac{x + \sqrt{-7}}{2} = \pm\beta^m$ and $\dfrac{x - \sqrt{-7}}{2} = \pm\alpha^m$.  Thus $\alpha^m - \beta^m = \pm\sqrt{-7}=\pm(\alpha-\beta)$.
We claim we cannot have $+$. Indeed, we have $\alpha+\beta=1$ and $\alpha\beta=2$. Thus $\alpha^2\equiv 1\mod \beta^2$. Thus $\alpha-\beta=\alpha^m-\beta^m \equiv \alpha \mod \beta^2$, nonsense. So $-\sqrt{-7} = \alpha^m-\beta^m$. Using the binomial expansion, we get $-2^{m-1}\equiv m\mod 7$. Since $m$ is also even, and the multiplicative order of $2$ modulo $7$ is three, we have that $m\equiv 3,5,13$ modulo $42$. We have solutions $m=3,5,13$. Suppose we have two solutions $m_1,m_2$ equivalent modulo $42$, with $m_2>m_1$. Write $k=m_2-m_1=6\cdot 7^l s$ where $7\nmid s$.
Now, we have 
\[(1+\sqrt{-7})^k \equiv 1+k\sqrt{-7} \mod 7^{l+1}\]
\[2^k\equiv 1\mod 7^{l+1}\]
So we get $k (\alpha^{m_1}+\beta^{m_1})\sqrt{-7} \equiv 0\mod 7^{l+1}$, which is easily a contradiction.
...
\begin{fact*}
Let $E$ be an $n$-dimensional Euclidean space, $\Gamma$ be a full lattice in $E$, and $X$ a centrally symmetric, convex subset of $E$. If moreover 
\[\vol(X) > 2^n \vol(\Gamma)\]
...
\begin{proof}
Suppose not. Let $P$ be a fundamental parallelepiped of $2\Gamma$. The map $\Phi : E\too E/(2\Gamma) \cong P$ is injective on $X$.
Then $X = \bigsqcup_{\gamma\in 2\Gamma} X_\gamma$ where $X_\gamma = (X \cap (P+\gamma))$. Thus $\Phi(X) = \bigsqcup_{\gamma\in 2\Gamma} \Phi(X_\gamma)$. However, $\vol(\Phi(X_\gamma)) = \vol(X_\gamma)$, so $\vol(X) = \vol(\Phi(X))\le \vol(P) = \vol(\Gamma)/2^n$.
\end{proof}


\begin{fact*}
Let $K$ be a number field of dimension $n$, and $\Phi : K \to \R^{r_1}\times \C^{r_2}\equiv R^n$ be its embeddings. Then $\Phi(\OO_K)$ is a lattice of volume $2^{-r_2}\sqrt{\disc_K}$. 
More generally, $\Phi(\a)$ is a lattice of volume $2^{-r_2}\sqrt{\disc_K}N(\a)$.
\end{fact*}


\begin{fact*}
Let $K$ be a number field, $\a$ a non-zero ideal in $\OO_K$. Then there is some $x\in\a$, $x\neq 0$, with $\abs{N_{K/\Q}(x)} \le M_K N(\a)$.
\end{fact*}


\begin{proof}
(Sketch). Work with the set $B_r = \{(x_1,\dots,x_{r_1},z_1,\dots,z_{r_2})\in\R^{r_1}\times\C^{r_2}:\sum \abs{x_i} + 2\sum\abs{z_j}\le r\}$.
\end{proof}


\newpage
\section{Unique Factorization to Prime Ideals}

\begin{fact*}
Let $K$ be a number field. Then any non-zero ideal $\a \ideal \OO_K$ may be written as a product of prime ideals
\[ \a = \p_1 \dots \p_k\]
...
\begin{fact*}
Let $K$ be a number field, $\a,\b,\c$ ideals of $\OO_K$. If $\a$ is non-zero, then \[\a\b=\a\c \implies \b=\c\]
\end{fact*}


\begin{fact*}
Let $K$ be a number field, $\a,\b$ ideals of $\OO_K$. Then 
\[\a \subseteq \b \iff \b \mid \a\]
\end{fact*}

\begin{fact*}
Let $K$ be a number field, $\a,\b,\p$ be ideals of $\OO_K$, where $\p$ is prime. Then 
\[\p \mid \a\b \implies \p \mid \a \text{ or } \p\mid \b\]
\end{fact*}

\begin{fact*}
Let $K$ be a number field. Then any ideal $\a$ of $\OO_K$ is divisible by some prime ideal $\p$ of $\OO_K$.
\end{fact*}

\begin{fact*}
Let $K$ be a number field. Then there is no infinite chain of ideals $\a_1 \subsetneq \a_2 \subsetneq \a_3 \subsetneq \dots$ of $\OO_K$.
\end{fact*}

\begin{fact*}
Let $K$ be a number field, $\a$ a non-zero ideal of $\OO_K$. Then there exists a non-zero ideal $\b$ of $\OO_K$ such that $\a\b$ is a principal ideal.
\end{fact*}
...
\end{enumerate}
for all $x,y\in K$. \\
If $\abs{x+y} \le \max(\abs{x},\abs{y})$ for all $x,y\in K$, we call $\abs{\cdot}$ \underline{non-Archimedean}. 
\end{nomn*}
...
\begin{fact*}
Let $\abs{\cdot}$ be a non-Archimedean absolute value. Then we have $\abs{x}\neq\abs{y} \implies \abs{x+y} = \max(\abs{x},\abs{y})$.
\end{fact*}


\begin{fact*}
Let $\abs{\cdot}$ be an absolute value on $K$. Then $\abs{\cdot}$ is non-Archimedean if and only if the integers are bounded (meaning $\abs{n} \le C$ for all integers $n$ and some constant $C$).
\end{fact*}
...
\begin{fact*}
Let $\abs{\cdot}$ be a non-Archimedean absolute value on $K$. Then any point in any ball is the center of the ball.
\end{fact*}


\begin{fact*}
Any absolute value on $\Q$ is either trivial, equivalent to the usual absolute value, or equivalent to the $p$-adic absolute value.
\end{fact*}


\begin{fact*}
Let $K$ be a complete field under an Archimedean absolute value $\abs{\cdot}$. Then $K,\abs{\cdot}$ is isomorphic to either $\R$ or $\C$ with the usual absolute value.
\end{fact*}


\begin{proof}
(Sketch). Suppose $K$ extends $\C$ with the usual absolute value, and $K\neq \C$. Let $z\in K\setminus\C$. We may find $\lambda\in\C$ closest to $z$, using compactness of closed balls in $\C$. Without loss of generality, we may assume $\lambda=0$. Set $m=\abs{z}$. So $m>0$ and $\abs{z-\alpha} \ge m$ for all $\alpha\in\C$. Now suppose $\abs{\alpha} < m$. Then $z^n-\alpha^n = \prod (z-\zeta^i \alpha)$ so $\abs{z-\alpha} m^{n-1} \le m^n + \abs{\alpha}^n$ so $\abs{z-\alpha} = m$. This clearly means $\abs{z-\alpha}=m$ for all $\alpha\in\C$, making $\abs{\cdot}$ bounded on $\C$.
\end{proof}


\begin{fact*}
Let $K$ be a complete field under the absolute value $\abs{\cdot}$. Suppose $E/K$ is a finite field extension. Then there is exactly one extension of the absolute value $\abs{\cdot}$ to an absolute value of $K$.
%(TODO show there is one). the proofs in the literature (eg https://math.stanford.edu/~conrad/676Page/handouts/ostrowski.pdf) uses Hensel's lemma and that complete Archimedean means R/C. 
\end{fact*}
...
\begin{fact*}
Suppose $L/K$ and $L'/K$ are Galois. Then so is $LL'/K$.
\end{fact*}


\begin{fact*}
Suppose $L/K$ is Galois, and $L'/K$ is finite. Then $[LL' : L'] = [L : L\cap L']$.
\end{fact*}


\begin{fact*}
Suppose two finite field extensions $L/K$ and $L'/K$ (inside some $L^{\alg}$) have $L\cap L'=K$, and at least one is Galois. Then $[LL' : K] = [L:K][L':K]$.
\end{fact*}
...
% taken from https://zevchonoles.org/math/pdf/course-notes/weinstein-analytic-class-number-formula.pdf
\begin{exer*}
Find formulas for $\zeta_K$ where $K=\Q(i)$. Deduce that $r_2(n) = \sum_{d\mid n}\chi(d)$ where $\chi$ is the non-trivial multiplicative character modulo four. Deduce the Gregory series.
\end{exer*}

\begin{exer*}
Find formulas for $\zeta_K$ where $K=\Q(\sqrt{-2})$. Deduce that $\sum_{n}\chi(n)/n = \pi/2^{3/2}$ where $\chi(n) = 1$ for $n\equiv 1,3\mod 8$ and $\chi(n) = -1$ for $n\equiv 5,7\mod 8$.
\end{exer*}


\begin{exer*}
Apply Minkowski's theorem to show the following. If $(-5:p)=1$, then either $p=x^2+5y^2$ or $2p=x^2+5y^2$.
\end{exer*}


\newpage
\section{Euclidean domains, PIDs, UFDs, Dedekind Domains}

\begin{nomn*}
A domain $A$ is \emph{Euclidean} if there is a function $\phi:A\setminus\{0\} \to \Z_{\ge0}$ such that for all $a,b\in A$ with $b$ non-zero, there exists $q,r$ in $A$ with \[a = bq + r\] and with \[r=0 \text{ or }\phi(r) < \phi(b)\]
\end{nomn*}

\begin{nomn*}
A \emph{Principal Ideal Domain} is a domain $A$ in which all ideals are principal.
\end{nomn*}
...
\begin{nomn*}
A \emph{Unique Factorization Domain} is a domain $A$ such that for each non-zero, non-unit $a\in A$, there exists a factorization
\[ a = p_1\dots p_k\]
...
\begin{proof}
Let $A$ be a PID, so $A$ is Noetherian, so we cannot have a strictly increasing infinite sequence of ideals of $A$. This implies that any non-zero, non-unit element $a$ is given as a product of irreducibles. Now it remains to show that if $p$ is irreducible then it is prime. Indeed, $\gen{p}$ is maximal among principal ideals, so it is a maximal ideal, hence a prime ideal, so $p$ is prime.
\end{proof}
...
An example of a UFD which is not a PID is $\Z[x]$. The ideal $\gen{2,x}$ is not principal.


Let us axiomatize the theory of unique factorization into prime ideals in the ring of integers of a number field.
...
\begin{fact*}
In a Dedekind domain, there is unique factorization of non-zero ideals into prime ideals.
\end{fact*}

\begin{fact*}
Let $A$ be a Dedekind domain. Then any ideal $\a$ of $A$ is generated by at most two elements. In fact, for any non-zero element $\alpha$ in $\a$ there is a complementary generator.
\end{fact*}

\begin{proof}
Let $\a\ideal A$ be a given non-zero ideal, and write $\a = \p_1^{e_1}\dots\p_n^{e_n}$. Given any non-zero $\alpha\in\a$, we write $(\alpha) = \p_1^{f_1}\dots p_n^{f_n}\q_1^{g_1}\dots\q_m^{g_m}$. We have $(\alpha)\subseteq \a$ so $\a\mid(\alpha)$ so $f_i\ge e_i$. Now suppose we've found some $\beta\in A$ with $\nu_{\p_i}(\beta)=e_i$ and $\nu_{\q_j}(\beta)=0$. Then we'd have $(\alpha,\beta)=\gcd((\alpha),(\beta))=\p_1^{e_1}\dots\p_n^{e_n}=\a$. So it remains to find such $\beta$. To do so, pick some $\gamma_i\in\p_i^{e_i}\setminus\p_i^{e_i+1}$. By the chinese remainder theorem, we may solve $\beta\equiv \gamma_i \mod \p_i^{e_i+1}$, $\beta\equiv 1\mod \q_j$.
\end{proof}

\begin{fact*}
Let $A$ be a Dedekind domain with field of fractions $K$, let $L/K$ be a finite field extension. Then the integral closure $B$ of $A$ in $L$ is a Dedekind domain, and its field of fractions is $L$.
\end{fact*}

\begin{fact*}
If $L/K$ is Galois, then $\Gal(L/K)$ acts transitively on the primes $\Pp$ over $\p$.
\end{fact*}
...
%   good refs:  Neukirch, Swinnerton-Dyer
%               http://gaetan.chenevier.perso.math.cnrs.fr/GT/ireland_rosen.pdf
%               https://virtualmath1.stanford.edu/~conrad/154Page/handouts.html
%               Ben Green, Brian Conrad, Keith Conrad, De Shalit
%               https://wstein.org/edu/Spring2004/129/ant/ant.pdf
%               https://web.archive.org/web/20250521201558/http://alpha.math.uga.edu/~pete/4400FULL.pdf
...
%               https://www.math.utoronto.ca/~ila/Algebraic%20Number%20Fields%20(%20PDFDrive%20).pdf
%               https://archive.math.arizona.edu/cais/scans/Cassels-Frohlich-Algebraic_Number_Theory.pdf
%               https://kskedlaya.org/papers/cft-ptx.pdf
...
%           algebraic geometry - https://users.math.msu.edu/users/magyarp/Math419H/Fulton-Alg-Curves.pdf

%           newton polygons https://personal.math.ubc.ca/~cass/research/pdf/Newton.pdf

%           algebraic proof of finiteness of class group in Ireland Rosen page 178



%           class field theory notes: 
%           Dexter
%           https://library.oapen.org/bitstream/id/35ae2ad8-672b-4bbf-ae41-b22c756d13da/9789493296039.pdf
%           https://www.eliothodges.com/notes.html
%           https://people.math.harvard.edu/~hamann/Course_Notes_CFT_Week_5.pdf
%           https://kskedlaya.org/papers/cft-ptx.pdf
